...
// Timing for test command
unsigned long last_test_command_time = 0;
...
// =============================================================================
// Test Command Functions - CRC Verification
// =============================================================================

void SendTestCommand() {
  // This is the exact command from DJI documentation:
  // AA 1A 00 03 00 00 00 00 22 11 A2 42 0E 00 20 00 30 00 40 00 01 14 7B 40 97 BE
  
  uint8_t expected_packet[26] = {
    0xAA, 0x1A, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x22, 0x11,  // Header (10 bytes)
    0xA2, 0x42,                                                   // Expected CRC16
    0x0E, 0x00,                                                   // Payload start
    0x20, 0x00, 0x30, 0x00, 0x40, 0x00, 0x01, 0x14,             // Payload data
    0x7B, 0x40, 0x97, 0xBE                                        // Expected CRC32
  };
  
  // Build packet step by step to verify our CRC functions
  uint8_t constructed_packet[26];
  uint16_t packet_index = 0;
  
  // Header (10 bytes)
  constructed_packet[packet_index++] = 0xAA;  // Start marker
  constructed_packet[packet_index++] = 0x1A;  // Length (26 bytes)
  constructed_packet[packet_index++] = 0x00;  // Reserved
  constructed_packet[packet_index++] = 0x03;  // Command type
  constructed_packet[packet_index++] = 0x00;  // Reserved
  constructed_packet[packet_index++] = 0x00;  // Reserved
  constructed_packet[packet_index++] = 0x00;  // Reserved
  constructed_packet[packet_index++] = 0x00;  // Reserved
  constructed_packet[packet_index++] = 0x22;  // Device address
  constructed_packet[packet_index++] = 0x11;  // Source address
  
  // Calculate CRC16 for header (first 10 bytes)
  const uint16_t calculated_crc16 = CalculateCrc16Dji(constructed_packet, 10);
  const uint16_t expected_crc16 = 0x42A2;  // Little-endian: A2 42
  
  constructed_packet[packet_index++] = calculated_crc16 & 0xFF;         // CRC16 low
  constructed_packet[packet_index++] = (calculated_crc16 >> 8) & 0xFF;  // CRC16 high
  
  // Payload (10 bytes)
  constructed_packet[packet_index++] = 0x0E;  // Payload length
  constructed_packet[packet_index++] = 0x00;  // Command set (should be 0x0E, but example shows 0x00)
  constructed_packet[packet_index++] = 0x20;  // Yaw low (0x0020 = 32 = 3.2°)
  constructed_packet[packet_index++] = 0x00;  // Yaw high
  constructed_packet[packet_index++] = 0x30;  // Roll low (0x0030 = 48 = 4.8°)
  constructed_packet[packet_index++] = 0x00;  // Roll high
  constructed_packet[packet_index++] = 0x40;  // Pitch low (0x0040 = 64 = 6.4°)
  constructed_packet[packet_index++] = 0x00;  // Pitch high
  constructed_packet[packet_index++] = 0x01;  // Control mode
  constructed_packet[packet_index++] = 0x14;  // Action time (20 = 2.0s)
  
  // Calculate CRC32 for entire packet (first 22 bytes)
  const uint32_t calculated_crc32 = CalculateCrc32Dji(constructed_packet, 22);
  const uint32_t expected_crc32 = 0xBE97407B;  // Little-endian: 7B 40 97 BE
  
  constructed_packet[packet_index++] = calculated_crc32 & 0xFF;          // CRC32 byte 0
  constructed_packet[packet_index++] = (calculated_crc32 >> 8) & 0xFF;   // CRC32 byte 1
  constructed_packet[packet_index++] = (calculated_crc32 >> 16) & 0xFF;  // CRC32 byte 2
  constructed_packet[packet_index++] = (calculated_crc32 >> 24) & 0xFF;  // CRC32 byte 3
  
  // Verify our calculations match the expected values
  Serial.println("=== CRC Verification ===");
  Serial.printf("Expected CRC16: 0x%04X, Calculated: 0x%04X %s\n", 
                expected_crc16, calculated_crc16, 
                (expected_crc16 == calculated_crc16) ? "✓ MATCH" : "✗ MISMATCH");
                
  Serial.printf("Expected CRC32: 0x%08X, Calculated: 0x%08X %s\n", 
                expected_crc32, calculated_crc32, 
                (expected_crc32 == calculated_crc32) ? "✓ MATCH" : "✗ MISMATCH");
  ...
  for (int i = 0; i < 26; i++) {
    Serial.printf("%02X ", constructed_packet[i]);
  }
  Serial.println();
  
  // Check if packets match
  bool packets_match = true;
  for (int i = 0; i < 26; i++) {
    if (expected_packet[i] != constructed_packet[i]) {
      packets_match = false;
      break;
    }
  }
  
  Serial.printf("Packet match: %s\n", packets_match ? "✓ PERFECT" : "✗ DIFFERENCE");
  
  // Send the constructed packet
  Serial.println("\nSending test command via CAN...");
  SendPacketOverCan(constructed_packet, 26);
  
  // Decode the command for verification
  const int16_t yaw = (constructed_packet[15] << 8) | constructed_packet[14];   // 0x0020 = 32
  const int16_t roll = (constructed_packet[17] << 8) | constructed_packet[16];  // 0x0030 = 48
  const int16_t pitch = (constructed_packet[19] << 8) | constructed_packet[18]; // 0x0040 = 64
  
  Serial.printf("Command: Yaw=%.1f°, Roll=%.1f°, Pitch=%.1f°, Mode=%d, Time=%.1fs\n",
                yaw/10.0f, roll/10.0f, pitch/10.0f, 
                constructed_packet[20], constructed_packet[21]/10.0f);
  Serial.println();
}
...
void ProcessParameterPush(const uint8_t *packet_data, uint8_t packet_length)
{
  ...
  if (packet_length < 35)
  { // Minimum size for parameter push
    return;
  }
...
void setup()
{
  ...
  Serial.println("=== DJI CRC Verification & Parameter Push Test ===");
  Serial.println("This test:");
  Serial.println("1. Verifies CRC calculations against known DJI command");
  Serial.println("2. Sends test command every 5 seconds");
  Serial.println("3. Enables parameter push and monitors frequency");
  Serial.println();
  ...
  // Send initial test command to verify CRC immediately
  SendTestCommand();
}
...
void loop()
{
  ...
  // Send test command every 5 seconds for CRC verification
  if (millis() - last_test_command_time >= kSendIntervalMs)
  {
    last_test_command_time = millis();
    SendTestCommand();
  }
